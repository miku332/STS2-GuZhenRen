...
public sealed class RecipeWanXingFeiYing : AbstractRecipeRelic
{
    private static readonly IReadOnlyList<RecipeIngredient> RequiredIngredients =
    [
        new(
            "ASSEMBLE_WAN_XING_XING_NIAN",
            static card => card is XingNianGu { IsUpgraded: true }),
        new(
            "ASSEMBLE_WAN_XING_XIAN_GU",
            static card =>
                card is GuZhenRenCardTemplate { Rank: >= 6 }
                && card.Tags.Contains(GuZhenRenTags.ZhiDao))
    ];

    internal override CardModel RewardCard => ModelDb.Card<WanXingFeiYing>();
