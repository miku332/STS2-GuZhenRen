...
public sealed class RecipeShangFangJieWa : AbstractRecipeRelic
{
    private static readonly IReadOnlyList<RecipeIngredient> RequiredIngredients =
    [
        new(
            "ASSEMBLE_SHANG_FANG_JIE_WA_HENG_CHONG",
            static card => card is HengChongZhiZhuangGu { IsUpgraded: true }),
        new(
            "ASSEMBLE_SHANG_FANG_JIE_WA_XIAN_GU",
            static card =>
                card is GuZhenRenCardTemplate { Rank: >= 6 and <= 9 }
                && GuZhenRenTagRules.HasEffectiveTag(card, GuZhenRenTags.LiDao))
    ];

    internal override CardModel RewardCard => ModelDb.Card<ShangFangJieWa>();
