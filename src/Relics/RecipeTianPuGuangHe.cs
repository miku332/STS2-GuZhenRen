...
public sealed class RecipeTianPuGuangHe : AbstractRecipeRelic
{
    private static readonly IReadOnlyList<RecipeIngredient> RequiredIngredients =
    [
        new(
            "ASSEMBLE_TIAN_PU_GUANG_HE_TAI_GUANG",
            static card => card is TaiGuangGu { IsUpgraded: true }),
        new(
            "ASSEMBLE_TIAN_PU_GUANG_HE_XIAN_GU",
            static card =>
                card is GuZhenRenCardTemplate { Rank: >= 1 and <= 9 }
                && GuZhenRenTagRules.HasEffectiveTag(card, GuZhenRenTags.GuangDao))
    ];

    internal override CardModel RewardCard => ModelDb.Card<TianPuGuangHe>();
